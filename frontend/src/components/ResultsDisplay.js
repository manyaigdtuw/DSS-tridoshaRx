...
const ResultsDisplay = ({ results, searchType, enteredSymptoms }) => {
  ...
  const getMatchCount = (diseaseSymptoms) => {
    if (!enteredSymptoms || enteredSymptoms.length === 0) return { matches: 0, total: 0 };
    const lowerEntered = enteredSymptoms.map(s => s.toLowerCase());
    const matches = diseaseSymptoms.filter(s => lowerEntered.includes(s.toLowerCase())).length;
    return { matches, total: enteredSymptoms.length };
  };
            ...
            <div key={index} className="result-card">
              ...
              {result.disease && (
                <div className="result-section">
                  ...
                  <div className="disease-name">{formatDisease(result.disease)}</div>
                  {searchType === 'partial' && total > 0 && (
                    <div
                      className="match-info"
                      style={{ fontSize: '0.9rem', color: '#666', marginTop: '8px' }}
                    >
                      {matches}/{total} symptoms matched
                    </div>
                  )}
                </div>
              )}
